 ...
 */
const functionDefinition = async function RemoveMeasure(request) {
  request.on("data", async bundle => {
    try {
      ...
      logger.error(`Error:${JSON.stringify(err)}`, {
        service: `NodeSseService::RemoveMeasure()`
      });
    }
  });
};

const DoRemoveMeasure = async function DoRemoveMeasure({ qsApp, id, commonHeader }) {
  let retVal = "";
  ...
  try {
    ...
    const result = await app.destroyMeasure({ qId: id });
    if (result) {
      logger.info("Measure: " + id + " Removed", {
        service: `NodeSseService::DoRemoveMeasure()`,
        app: appId
      });
    } else {
      logger.info("Measure: " + id + " not removed", {
        service: `NodeSseService::DoRemoveMeasure()`,
        app: appId
      });
    }
